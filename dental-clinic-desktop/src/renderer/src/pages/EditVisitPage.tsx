...
export function EditVisitPage(): JSX.Element {
  ...
  useEffect(() => {
    if (!visitId) return
    api.visits.getWithMedicines(visitId).then(visit => {
      if (!visit) {
      ...
      setForm({
        ...
        chief_complaint: visit.chief_complaint, diagnosis: visit.diagnosis, treatment_done: visit.treatment_done,
        clinical_notes: visit.clinical_notes, advice: visit.advice, next_visit_date: visit.next_visit_date,
        fee_charged: visit.fee_charged, amount_paid: visit.amount_paid
      })
      setMedicines(visit.medicines.length > 0 ? visit.medicines : [{ ...EMPTY_MED }])
      setPatientId(visit.patient_id)
      
      api.patients.getById(visit.patient_id).then(p => {
        setPatient(p)
        setLoading(false)
      })
    }).catch(err => {
  ...
  async function handleSubmit(): Promise<void> {
    ...
    try {
      await api.visits.update(visitId, {
        ...form,
  ...
  if (loading) return <div className="workspace"><p>Loading visit...</p></div>
  if (!patient) return <div className="workspace"><p>Patient not found.</p></div>
    ...
    <div className="workspace">
      <div className="page-header">
        <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
          <button className="btn btn-ghost" onClick={() => navigate(`/visits/${visitId}`)}>← Back to Visit</button>
          <div><h2 className="page-title">Edit Visit</h2><div className="page-subtitle">{patient.full_name} — #{patient.patient_number}</div></div>
        </div>
      </div>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 16, maxWidth: 900 }}>
        ...
        <div className="btn-row">
          <button className="btn btn-primary" disabled={saving} onClick={handleSubmit}>{saving ? 'Saving...' : 'Save Changes'}</button>
          <button className="btn btn-ghost" onClick={() => navigate(`/visits/${visitId}`)} disabled={saving}>Cancel</button>
        </div>
      </div>
    </div>
  )
